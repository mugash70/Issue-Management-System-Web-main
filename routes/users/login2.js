        ...
        await pool.query(`SELECT * FROM employees WHERE email = $1`, [email], (err, response) => {
            ...
            const validateUser = () => {
                ...
                if (userEmail) {
                    ...
                    next(new Error.res.status(400).json({ msg: "Invalid user" }));
                }
                if (userPassword) {
                    return userPassword
                }
            ...
            if (err) {
                ...
                if (response.rows.length === 0) {
                    ...
                    let hash = response.rows[0].temporary_pw; //pw in db
                    bcrypt.compare(req.body.temporary_pw, hash, function (err, result) {
                        ...
                        if (result) {
                            ...
                            res.status(200).json({
                                token,
                                user: { id: response.rows[0].emp_id, email: response.rows[0].email, name: response.rows[0].name }
                            });
                        }
                        else {
